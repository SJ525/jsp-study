...
//实现考生的登陆功能
@WebServlet("/handleStuLogin")
public class HandleStuLogin extends HttpServlet {
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response) 
		...
		if(ok==true&&account.equals(loginBean.getAccount())){
           ...
           try{ 
                ...
                String condition="select * from tb_student where account = '"+account+
		...
		if(loginBean.getSuccess()==true) {
            ...
            request.getRequestDispatcher("/stu/indexStu.jsp");
            dispatcher.forward(request,response);
        }else {
     	   //登录失败，转发到考生登陆首页
            RequestDispatcher dispatcher=
            request.getRequestDispatcher("loginStu.jsp");
            dispatcher.forward(request,response);
        } 
   }
